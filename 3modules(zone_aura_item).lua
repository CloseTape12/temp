...
-- ── Discord Webhook для оповещений ──────────────────────────────
local DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1229864317982212117/b0dcQJrOsLWFR25nfjxi33zyz7Tr98xVpvnUvsoi-T9_uzJgja20YEYjq3DKR0O59GWq" -- Замени на свой webhook URL

-- Функция отправки сообщения на Discord
local function sendDiscordNotification(title, description, color)
    if DISCORD_WEBHOOK_URL == "YOUR_DISCORD_WEBHOOK_URL_HERE" then
        warn("⚠️ Discord webhook не настроен! Установи URL в переменной DISCORD_WEBHOOK_URL")
        return
    end
...
sendDiscordNotification("✅ SLIMES Bot Запущен", "Скрипт успешно включен! Все три модуля работают:\n• Авто-лутер\n• Килл-аура\n• Авто-покупка локаций", 3066993)
